...
void TestAxisRaysAabb(const Aabb& aabb, float result[],
                      bool collision_on_exit) {
  // These are inward facing rays, which should all hit.
  result[0] =
      CheckRayAABBCollision(Ray(mathfu::vec3(2, 0, 0), mathfu::vec3(-1, 0, 0)),
                            aabb, collision_on_exit);
  result[1] =
      CheckRayAABBCollision(Ray(mathfu::vec3(-2, 0, 0), mathfu::vec3(1, 0, 0)),
                            aabb, collision_on_exit);
  result[2] =
      CheckRayAABBCollision(Ray(mathfu::vec3(0, 2, 0), mathfu::vec3(0, -1, 0)),
                            aabb, collision_on_exit);
  result[3] =
      CheckRayAABBCollision(Ray(mathfu::vec3(0, -2, 0), mathfu::vec3(0, 1, 0)),
                            aabb, collision_on_exit);
  result[4] =
      CheckRayAABBCollision(Ray(mathfu::vec3(0, 0, 2), mathfu::vec3(0, 0, -1)),
                            aabb, collision_on_exit);
  result[5] =
      CheckRayAABBCollision(Ray(mathfu::vec3(0, 0, -2), mathfu::vec3(0, 0, 1)),
                            aabb, collision_on_exit);
...
TEST(CheckRayOBBCollision, Hit) {
  ...
  // Test untransformed AABB.
  dist = CheckRayAABBCollision(ray, aabb, false);
  EXPECT_NEAR(dist, 1.f, kEpsilon);
  dist = CheckRayAABBCollision(ray, aabb, true);
  EXPECT_NEAR(dist, 1.f, kEpsilon);
}
...
TEST(CheckRayOBBCollision, Thorough) {
  ...
  // Test axis aligned rays from each direction against the untransformed AABB.
  TestAxisRaysAabb(aabb, results, false);
  ...
  TestAxisRaysAabb(aabb, results, true);
...
TEST(GetHeading, Simple) {
  ...
  EXPECT_NEAR(0.0, mathfu::vec3::Distance(axis, mathfu::kAxisY3f), kEpsilon);
