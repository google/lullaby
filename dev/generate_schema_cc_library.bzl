...
def generate_schema_cc_library(
  ...
  Args:
    ...
    out_prefix: Path to prefix to generated outputs.
    includes: Optional, list of filegroups of schemas that the srcs depend on.
        ** SEE REMARKS BELOW **
    include_paths: Optional, list of paths the includes files can be found in.
    filegroup_name: DEPRECATED Name of filegroup containing the schema files.
        If None, then this will be "{name}_includes".
  ...
  Outs:
    ...
    filegroup(filegroup_name): As name+"_includes", but using the deprecated
        custom filegroup_name.  Will be removed as part of b/80082898.
  ...
  for src in srcs:
    ...
    for label, generator, args, out_path, suffix in cmds:
      ...
      native.genrule(name="generate_%s_code_%s" % (label, basename),
                     srcs = [src],
                     outs = [out],
                     tools = full_includes + [generator],
                     cmd = " ".join(cmd),
  ...
  native.cc_library(name = name,
                    ...
                    includes = [out_prefix if out_prefix else "."],
                    linkstatic = 1,
  ...
  # DEPRECATED: b/80082898 remove once all uses of filegroup_name are refactored.
  if filegroup_name and filegroup_name != "%s_includes" % (name):
    native.filegroup(
        name = filegroup_name,
        srcs = srcs,
        visibility = visibility)
