...
TEST_F(TransformSystemTest, AddChildPreserveWorldToEntity) {
  ...
  transform_system->AddChild(
      parent_2, child,
      TransformSystem::ModifyParentChildMode::kPreserveWorldToEntityTransform);
...
TEST_F(TransformSystemTest, RemoveParent) {
  auto* transform_system = registry_.Get<TransformSystem>();
  const Entity child = 1;
  const Entity parent_1 = 2;
  ...
  // Child has changed world pose to match local sqt.
  sqt =
      CalculateSqtFromMatrix(transform_system->GetWorldFromEntityMatrix(child));
  EXPECT_THAT(sqt.translation, NearMathfuVec3({1.f, 2.f, 3.f}, kEpsilon));
  EXPECT_THAT(sqt.rotation.ToEulerAngles(),
              NearMathfuVec3(mathfu::vec3(90.f, 0.f, 0.f) * kDegreesToRadians,
                             kEpsilon));
  EXPECT_THAT(sqt.scale, NearMathfuVec3({2.f, 3.f, 4.f}, kEpsilon));

  // Child has not changed local sqt.
  sqt = *transform_system->GetSqt(child);
  EXPECT_THAT(sqt.translation, NearMathfuVec3({1.f, 2.f, 3.f}, kEpsilon));
  EXPECT_THAT(sqt.rotation.ToEulerAngles(),
              NearMathfuVec3(mathfu::vec3(90.f, 0.f, 0.f) * kDegreesToRadians,
...
TEST_F(TransformSystemTest, RemoveParentPreserveWorldToEntity) {
  auto* transform_system = registry_.Get<TransformSystem>();
  const Entity child = 1;
  const Entity parent_1 = 2;
  ...
  // Child has new matching local sqt.
  sqt = *transform_system->GetSqt(child);
  EXPECT_THAT(sqt.translation, NearMathfuVec3({3.f, -10.f, 9.f}, kEpsilon));
  EXPECT_THAT(sqt.rotation.ToEulerAngles(),
              NearMathfuVec3(mathfu::vec3(180.f, 0.f, 0.f) * kDegreesToRadians,
...
TEST_F(TransformSystemTest, EnableDisable_AddChild) {
  for (int i = 1; i <= 5; ++i) {
    CreateDefaultTransform(i);
  }
  auto* transform_system = registry_.Get<TransformSystem>();
  transform_system->Disable(1);
  transform_system->AddChild(3, 4);
  transform_system->AddChild(1, 3);
  EXPECT_THAT(transform_system->IsEnabled(3), Eq(false));
  EXPECT_THAT(transform_system->IsLocallyEnabled(3), Eq(true));
  EXPECT_THAT(transform_system->IsEnabled(4), Eq(false));
  EXPECT_THAT(transform_system->IsLocallyEnabled(4), Eq(true));

  transform_system->AddChild(2, 3);
  EXPECT_THAT(transform_system->IsEnabled(3), Eq(true));
  EXPECT_THAT(transform_system->IsLocallyEnabled(3), Eq(true));
  EXPECT_THAT(transform_system->IsEnabled(4), Eq(true));
  EXPECT_THAT(transform_system->IsLocallyEnabled(4), Eq(true));

  transform_system->Disable(5);
  transform_system->AddChild(1, 5);
  EXPECT_THAT(transform_system->IsEnabled(5), Eq(false));
  EXPECT_THAT(transform_system->IsLocallyEnabled(5), Eq(false));

  transform_system->AddChild(2, 5);
  EXPECT_THAT(transform_system->IsEnabled(5), Eq(false));
  EXPECT_THAT(transform_system->IsLocallyEnabled(5), Eq(false));
}

TEST_F(TransformSystemTest, EnableDisable_RemoveParent) {
  for (int i = 1; i <= 3; ++i) {
    CreateDefaultTransform(i);
  }
  auto* transform_system = registry_.Get<TransformSystem>();
  ...
  transform_system->RemoveParent(2);
  EXPECT_THAT(transform_system->IsEnabled(2), Eq(true));
  EXPECT_THAT(transform_system->IsLocallyEnabled(2), Eq(true));

  transform_system->Disable(3);
  transform_system->AddChild(1, 3);
  EXPECT_THAT(transform_system->IsEnabled(3), Eq(false));
  EXPECT_THAT(transform_system->IsLocallyEnabled(3), Eq(false));

  transform_system->RemoveParent(3);
  EXPECT_THAT(transform_system->IsEnabled(3), Eq(false));
  EXPECT_THAT(transform_system->IsLocallyEnabled(3), Eq(false));
}
