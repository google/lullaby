...
void CodeGenerator::GenerateStructDecl(const flatbuffers::StructDef& def) {
  ...
  if (parser_.file_identifier_.length()) {
    code_.SetValue("FILE_ID", parser_.file_identifier_);
    code_ += "";
    code_ += "  static const char* FileIdentifier() {";
    code_ += "    return \"{{FILE_ID}}\";";
    code_ += "  }";
  } else {
    code_ += "";
    code_ += "  static const char* FileIdentifier() {";
...
void CodeGenerator::GenerateInclude(const std::string& fullname) {
  const std::string basename = parser_.opts.keep_include_path ?
      flatbuffers::StripExtension(fullname) :
