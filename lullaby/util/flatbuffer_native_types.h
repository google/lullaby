...
#ifndef LULLABY_UTIL_FLATBUFFER_NATIVE_TYPES_H_
#define LULLABY_UTIL_FLATBUFFER_NATIVE_TYPES_H_
...
// Specialization for mathfu::AffineTransform which has a Flatbuffer type of
// lull::Mat4x3.
template <>
struct FlatbufferNativeType<mathfu::AffineTransform> {
  static constexpr size_t kFlatbufferStructSize = 12 * sizeof(float);
  static constexpr size_t kFlatbufferStructAlignment = 4;

  static mathfu::AffineTransform Read(const void* src, size_t len) {
    mathfu::AffineTransform result;
    if (src && len >= kFlatbufferStructSize) {
      const float* values = reinterpret_cast<const float*>(src);
      for (int i = 0; i < 12; ++i) {
        result(i) = values[i];
      ...
      result = mathfu::AffineTransform::Identity();
    }
    return result;
  }

  static void Write(const mathfu::AffineTransform& src, void* dst, size_t len) {
    if (dst && len >= kFlatbufferStructSize) {
      float* values = reinterpret_cast<float*>(dst);
      for (int i = 0; i < 12; ++i) {
        values[i] = src[i];
...
#endif  // LULLABY_UTIL_FLATBUFFER_NATIVE_TYPES_H_
