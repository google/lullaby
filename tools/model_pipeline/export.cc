...
static constexpr bool kEmbedTextureData = true;

static std::string GetTextureName(const std::string& name) {
  if (kEmbedTextureData) {
...
void ExportModelInstance(const Model& model, ModelInstanceDefT* out,
  ...
  for (const Drawable& drawable : model.GetDrawables()) {
    ...
    for (auto& texture : drawable.material.textures) {
      if (!texture.second.source.empty()) {
        MaterialTextureDefT material_texture;
        material_texture.name = GetTextureName(texture.second.source);
        material_texture.usage = texture.second.usage;
        material_def.textures.emplace_back(material_texture);
      }
    }
...
void ExportTexture(const TextureInfo& info, TextureDefT* out,
                   TextureDefT* config) {
  const std::string& name = info.source;
  out->file = info.source;
  out->name = GetTextureName(name);
  ...
  *config = *out;
  ...
  if (kEmbedTextureData) {
    ...
    CHECK(file) << "Unable to open texture file: " << name;
