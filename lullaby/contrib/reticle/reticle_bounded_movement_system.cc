...
#include "lullaby/contrib/reticle/reticle_bounded_movement_system.h"
...
#if LULLABY_ENABLE_TEST_ENGINE
// Disables the elbow mode for testing.
static const float kDefaultControllerHeight = 0.0f;
#else
...
void ReticleBoundedMovementSystem::Enable(Entity entity) {
  ...
  registry_->Get<ReticleSystem>()->SetReticleMovementFn(fn);
}
