...
def build_entity_bin(
  ...
  # TODO(b/65735656) add schema+"_includes" to full includes when all apps are
  ...
  if jsonnet_library_srcs:
    ...
    jsonnet_library(name=jsonnet_library_name,
  ...
  for src in srcs:
    if src.endswith(".json"):
      ...
      jsonnet_to_json(name="jsonnet_to_json_%s" % label_suffix,
                      src=src,
                      outs=[out],
  ...
  for src in json_srcs:
    ...
    if basename in merged_srcs:
      ...
      if hasattr(native, "genmpm"):
        ...
        native.genrule(name="%s_merge_step" % (basename),
                       tools=[merge_script],
                       srcs=[basefile, src],
                       outs=[out],
                       cmd=" ".join([
                           ...
                           "-x $(location %s)" % src, "-o $(location %s)" % out
                           ]))
  ...
  for basename, src in merged_srcs.items():
    ...
    native.genrule(name="generate_entity_%s" % (label_suffix),
                   srcs=[src],
                   outs=[out],
                   ...
                   cmd=" ".join([
                       ...
                       " ".join(flatc_args),
                       " ".join(include_paths_cmd), "-o $(@D)",
                       "-b $(locations %s)" % (schema), "$(location %s)" % (src)
                   ]),
...
# TODO(b/31383797): Consider moving into flatbuffers build_defs.
#
# Example of use:
# load("//third_party/lullaby/dev/build_entity.bzl", "flatc_generate_binaries")
# flatc_generate_binaries(
#     name = "entities",
#     srcs = [":json_sources"],
#     include_paths = ["third_party/lullaby/schemas","third_party/lullaby/schemas"],
#     includes = ["//:fbs_schema_includes"],
