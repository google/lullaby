...
#ifndef REDUX_ENGINES_SCRIPT_REDUX_SCRIPT_PARSER_H_
#define REDUX_ENGINES_SCRIPT_REDUX_SCRIPT_PARSER_H_
...
struct ParserCallbacks {
  ...
  virtual void Process(TokenType type, const void* ptr,
                       std::string_view token) = 0;
  virtual void Error(std::string_view token, std::string_view message) = 0;
};

// Parses Script source code, invoking the callback function as individual
// tokens are extracted.
void ParseScript(std::string_view source, ParserCallbacks* callbacks);

}  // namespace redux

#endif  // REDUX_ENGINES_SCRIPT_REDUX_SCRIPT_PARSER_H_
