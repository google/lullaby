...
TEST_F(CollisionSystemTest, EventEnableDisable) {
  TransformDefT transform;
  CollisionDefT collision;
  ...
  DisableCollisionEvent disable_event;
  disable_event.entity = entity;
  dispatcher->Send(disable_event);
  EXPECT_FALSE(collision_system->IsCollisionEnabled(entity));
  EnableCollisionEvent enable_event;
  enable_event.entity = entity;
  dispatcher->Send(enable_event);
  EXPECT_TRUE(collision_system->IsCollisionEnabled(entity));
}
...
TEST_F(CollisionSystemTest, CheckForCollisionReverseOrder) {
  Blueprint blueprint1;
  {
    TransformDefT transform;
    transform.position = mathfu::vec3(0.f, 0.f, -4.f);
    transform.aabb = Aabb(-mathfu::kOnes3f, mathfu::kOnes3f);
    CollisionDefT collision;
    blueprint1.Write(&transform);
    blueprint1.Write(&collision);
  }
  Blueprint blueprint2;
  {
    TransformDefT transform;
    transform.position = mathfu::vec3(0.f, 0.f, -2.f);
    transform.aabb = Aabb(-mathfu::kOnes3f / 2.f, mathfu::kOnes3f / 2.f);
    CollisionDefT collision;
    blueprint2.Write(&transform);
    blueprint2.Write(&collision);
  }
  ...
  // Create the closer entity first
  const Entity entity2 = entity_factory->Create(&blueprint2);
  const Entity entity1 = entity_factory->Create(&blueprint1);
  EXPECT_NE(entity1, kNullEntity);
  EXPECT_NE(entity2, kNullEntity);
  ...
  // Shoot a ray that will hit both entities, returning the closer entity.
  {
    const auto result = collision_system->CheckForCollision(ray);
    EXPECT_EQ(result.entity, entity2);
    EXPECT_NEAR(result.distance, 1.5f, kEpsilon);
  }
}

TEST_F(CollisionSystemTest, CollisionOnExit) {
  Blueprint blueprint1;
  {
    TransformDefT transform;
    transform.position = mathfu::vec3(0.f, 0.f, -4.f);
    transform.aabb = Aabb(-mathfu::kOnes3f, mathfu::kOnes3f);
    CollisionDefT collision;
    collision.collision_on_exit = true;
    blueprint1.Write(&transform);
    blueprint1.Write(&collision);
  }
  Blueprint blueprint2;
  ...
  EXPECT_NE(entity1, kNullEntity);
  EXPECT_TRUE(collision_system->IsCollisionEnabled(entity1));
  // Shoot a ray that will hit the back of entity 1.
  {
    const auto result = collision_system->CheckForCollision(ray);
    EXPECT_EQ(result.entity, entity1);
    EXPECT_NEAR(result.distance, 5.f, kEpsilon);
  }

  const Entity entity2 = entity_factory->Create(&blueprint2);
  EXPECT_NE(entity2, kNullEntity);
  EXPECT_TRUE(collision_system->IsCollisionEnabled(entity2));
  // Shoot a ray that will hit both entities, returning entity 2 because even
  // though entity 1 is bigger, it only collides in the back.
  {
    const auto result = collision_system->CheckForCollision(ray);
    EXPECT_EQ(result.entity, entity2);
  ...
  const Entity entity3 = entity_factory->Create(&blueprint3);
  EXPECT_NE(entity3, kNullEntity);
  EXPECT_TRUE(collision_system->IsCollisionEnabled(entity3));
  // Confirm that entity 3 with the same size as entity 1 will normally block
  // entity 2 since it does not have collision_on_exit.
  {
    const auto result = collision_system->CheckForCollision(ray);
    EXPECT_EQ(result.entity, entity3);
    EXPECT_NEAR(result.distance, 3.f, kEpsilon);
  }
}
...
TEST_F(CollisionSystemTest, CheckForClip) {
  ...
  {
    TransformDefT transform;
    spacer_blueprint.Write(&transform);
  }
  {
    TransformDefT transform;
  ...
  const Entity spacer = entity_factory->Create(&spacer_blueprint);
  const Entity child = entity_factory->Create(&child_blueprint);
  // Add a spacer so ancestry check has to recurse at least once. It doesn't
  // change any positions.
  transform_system->AddChild(parent, spacer);
  transform_system->AddChild(spacer, child);

  EXPECT_NE(parent, kNullEntity);
  EXPECT_NE(spacer, kNullEntity);
  EXPECT_NE(child, kNullEntity);
  EXPECT_TRUE(collision_system->IsCollisionEnabled(child));

  static const float kEpsilon = 0.001f;
  const Ray inside(mathfu::vec3(4.5f, 4.5f, 0.f), -mathfu::kAxisZ3f);
  const Ray outside(mathfu::vec3(4.75f, 4.75f, 0.f), -mathfu::kAxisZ3f);

  // Shoot a ray that will hit the child inside the bounds.
  {
    const auto result = collision_system->CheckForCollision(inside);
  ...
  {
    const auto result = collision_system->CheckForCollision(outside);
    EXPECT_EQ(result.entity, kNullEntity);
    EXPECT_EQ(result.distance, kNoHitDistance);
  }

  // Disable collision clipping, then shoot the same ray and expect a collision.
  ...
  // Re-enable and expect the previous result.
  collision_system->EnableClipping(child);
  {
    const auto result = collision_system->CheckForCollision(outside);
    EXPECT_EQ(result.entity, kNullEntity);
    EXPECT_EQ(result.distance, kNoHitDistance);
...
TEST_F(CollisionSystemTest, CheckForClipFailureCases) {
  auto* entity_factory = registry_->Get<EntityFactory>();
  auto* collision_system = registry_->Get<CollisionSystem>();
  auto* transform_system = registry_->Get<TransformSystem>();
  ...
  static const float kEpsilon = 0.001f;
  const Ray ray(mathfu::vec3(4.75f, 4.75f, 0.f), -mathfu::kAxisZ3f);

  // Shoot a ray that will hit the child event though it has clipping since no
  // bounds exist.
  {
    const auto result = collision_system->CheckForCollision(ray);
    EXPECT_EQ(result.entity, child);
...
TEST_F(CollisionSystemTest, DefaultInteractionDescendants) {
  ...
  const Entity child2 = entity_factory->Create(&blueprint);
  EXPECT_NE(child, kNullEntity);
  EXPECT_NE(child2, kNullEntity);

  EXPECT_TRUE(collision_system->IsCollisionEnabled(parent));
  EXPECT_TRUE(collision_system->IsInteractionEnabled(parent));

  transform_system->AddChild(parent, child);
  transform_system->AddChild(parent, child2);
  collision_system->DisableDefaultInteraction(child2);
  ...
  EXPECT_FALSE(collision_system->IsInteractionEnabled(child2));
