...
TEST(EventWrapper, SetConcreteVariant) {
  const Event event(123, "hello");
  EventWrapper wrapper(event);
  EXPECT_THAT(wrapper.GetTypeId(), Eq(GetTypeId<Event>()));
  EXPECT_THAT(wrapper.Get<Event>(), Not(IsNull()));
  ...
  // We cannot modify concrete events with variants, so these should not change.
  Variant number(456);
  Variant word(std::string("world"));
  wrapper.SetValue(kNumberHash, number);
  wrapper.SetValue(kWordHash, word);
  EXPECT_THAT(wrapper.GetTypeId(), Eq(GetTypeId<Event>()));
  EXPECT_THAT(wrapper.Get<Event>(), Not(IsNull()));
  const Event* ptr2 = wrapper.Get<Event>();
  EXPECT_THAT(event.number, Eq(ptr2->number));
  EXPECT_THAT(event.word, Eq(ptr2->word));

  VariantMap map;
  map[kNumberHash] = number;
  map[kWordHash] = word;
  wrapper.SetValues(map);
  EXPECT_THAT(wrapper.GetTypeId(), Eq(GetTypeId<Event>()));
  EXPECT_THAT(wrapper.Get<Event>(), Not(IsNull()));
  const Event* ptr3 = wrapper.Get<Event>();
  EXPECT_THAT(event.number, Eq(ptr3->number));
  EXPECT_THAT(event.word, Eq(ptr3->word));
}
