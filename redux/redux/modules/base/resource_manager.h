...
enum class ResourceCacheMode : uint8_t {
  kCacheFullyOnCreate,
  kWeakCachingOnly,
  kCacheExplicitly,
};
...
class ResourceManager {
 public:
  ...
  explicit ResourceManager(ResourceCacheMode mode) : mode_(mode) {}
  ...
  // Adds an internal strong reference to the resource referenced by the key.
  void Acquire(HashValue key);
 ...
 private:
  ...
  ResourceCacheMode mode_ = ResourceCacheMode::kWeakCachingOnly;
  absl::flat_hash_map<HashValue, ObjectCacheEntry> objects_;
...
typename ResourceManager<T>::ObjectPtr ResourceManager<T>::Create(
  ...
  if (obj && mode_ != ResourceCacheMode::kCacheExplicitly) {
    ObjectCacheEntry entry;
    entry.weak_ref = obj;
    if (mode_ == ResourceCacheMode::kCacheFullyOnCreate) {
...
template <typename T>
void ResourceManager<T>::Acquire(HashValue key) {
  auto iter = objects_.find(key);
  if (iter != objects_.end()) {
    iter->second.strong_ref = iter->second.weak_ref.lock();
  }
}
...
#endif  // REDUX_MODULES_BASE_RESOURCE_MANAGER_H_
