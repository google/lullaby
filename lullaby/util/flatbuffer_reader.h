...
#ifndef LULLABY_UTIL_FLATBUFFER_READER_H_
#define LULLABY_UTIL_FLATBUFFER_READER_H_
  ...
  void VectorOfScalars(std::vector<T>* value, uint16_t offset) {
    ...
    if (vec) {
      ...
      for (unsigned int i = 0; i < num; ++i) {
        value->at(i) = static_cast<T>(vec->Get(i));
      }
    } else {
      value->clear();
    }
...
#endif  // LULLABY_UTIL_FLATBUFFER_READER_H_
