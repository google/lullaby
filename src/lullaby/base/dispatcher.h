...
#ifndef LULLABY_BASE_DISPATCHER_H_
#define LULLABY_BASE_DISPATCHER_H_
...
// functor connected with the Event.  (The use of two functors is where the term
// "double-dispatch" gets its name.)
//
...
//
// Alternatively, clients can provide an additional "owner" const void* tag when
// connecting to a Dispatcher.  In this case, a non-scoped Connection object is
// returned.  The client can then either call Disconnect on the Connection
...
class Dispatcher {
 ...
 public:
  ...
  // automatically disconnect the connection when this object goes out of scope.
  ...
  // base-class Dispatcher does not queue any events, several subclasses benefit
  // from having such a function exposed as a base-class API.
 ...
 private:
  ...
  // Creates the actual Handler instance, registers it with the map, and returns
...
#endif  // LULLABY_BASE_DISPATCHER_H_
