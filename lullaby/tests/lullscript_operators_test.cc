...
#include "lullaby/tests/lullscript_testing_macros.h"
...
TEST(ScriptFunctionsOperatorsTest, ComparisonOperatorsEntity) {
  ScriptEnv env;
  ScriptValue res;

  LULLABY_TEST_SCRIPT_VALUE(env, "(== (Entity 1) (Entity 1))", true);
  LULLABY_TEST_SCRIPT_VALUE(env, "(== (Entity 1) (Entity 2))", false);
  LULLABY_TEST_SCRIPT_VALUE(env, "(!= (Entity 1) (Entity 1))", false);
  LULLABY_TEST_SCRIPT_VALUE(env, "(!= (Entity 1) (Entity 2))", true);
  LULLABY_TEST_SCRIPT_VALUE(env, "(== (Entity 1) 1)", true);
  LULLABY_TEST_SCRIPT_VALUE(env, "(!= (Entity 1) 2)", true);
  LULLABY_TEST_SCRIPT_VALUE(env, "(== 1 (Entity 1))", true);
  LULLABY_TEST_SCRIPT_VALUE(env, "(!= 1 (Entity 2))", true);
  LULLABY_TEST_SCRIPT_VALUE(env, "( < (Entity 1) (Entity 2))", true);
  LULLABY_TEST_SCRIPT_VALUE(env, "( < (Entity 1) (Entity 1))", false);
  LULLABY_TEST_SCRIPT_VALUE(env, "( < (Entity 2) (Entity 1))", false);
  LULLABY_TEST_SCRIPT_VALUE(env, "( < (Entity 1) 2)", true);
  LULLABY_TEST_SCRIPT_VALUE(env, "( < 1 (Entity 2))", true);
...
TEST(ScriptFunctionsOperatorsTest, MathOperatorsEntity) {
  ScriptEnv env;
  ScriptValue res;

  // These are only provided for legacy purposes, please do not introduce new
  // usages.
  LULLABY_TEST_SCRIPT_VALUE(env, "(+ (Entity 1) (Entity 1))", Entity(2));
  LULLABY_TEST_SCRIPT_VALUE(env, "(+ (Entity 1) 1)", Entity(2));
  LULLABY_TEST_SCRIPT_VALUE(env, "(+ 1 (Entity 1))", Entity(2));
  LULLABY_TEST_SCRIPT_VALUE(env, "(% (Entity 13) (Entity 5))", Entity(3));
  LULLABY_TEST_SCRIPT_VALUE(env, "(% (Entity 13) 5)", Entity(3));
  LULLABY_TEST_SCRIPT_VALUE(env, "(% 13 (Entity 5))", Entity(3));
}
