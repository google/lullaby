...
int main(int argc, const char** argv) {
  ...
  flatbuffers::DetachedBuffer fb = lull::tool::BuildShader(shader_build_params);
  if (!fb.data()) {
  ...
  const char* out_path = parser.GetString("out").c_str();
  if (!lull::tool::SaveFile(fb.data(), fb.size(), out_path,
                            /* binary = */ true)) {
