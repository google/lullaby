...
load("//third_party/fplbase:build_defs.bzl",
     "fpl_png_assets",)
...
def _lullaby_etc_internal(name, out, src, srcs, fmt, effort, create_mips):
  ...
  native.genrule(name="%s_%s" % (name, src),
                 srcs=srcs,
                 outs=[out],
                 tools=[etctool_path],
                 cmd=" ".join([
...
def build_cubemap_texture(
  ...
  convert_path = "//third_party/ImageMagick6:convert"

  ext = out.split(".")[-1]

  stacked_image = out.replace(ext, "png")

  native.genrule(name=name + "_stack",
                 srcs=srcs.values(),
                 outs=[stacked_image],
                 tools=[convert_path],
                 cmd=" ".join([
