...
FontPtr FlatuiTextSystem::LoadFonts(const std::vector<std::string>& names) {
  return std::make_shared<Font>(&font_manager_, names);
}
...
void FlatuiTextSystem::GenerateText(Entity entity, Entity desired_size_source) {
  ...
  const std::string processed_text = preprocessor
      ? preprocessor->ProcessString(component->text)
      : component->text;
