...
TEST(ArgParserTest, Positional) {
  ...
  EXPECT_THAT(position_args[0], Eq(args[0]));
  EXPECT_THAT(position_args[1], Eq(args[1]));
  EXPECT_THAT(position_args[2], Eq(args[2]));
}
...
TEST(ArgParserTest, MultipleArgs) {
  ...
  const char* args[] = {"--test", "foo", "--bar", "baz"};
  const int num_args = sizeof(args) / sizeof(args[0]);
  const bool result = parser.Parse(num_args, args);
  EXPECT_TRUE(result);
  ...
  EXPECT_THAT(position_args[0], Eq(args[3]));
}
...
TEST(ArgParserTest, MultipleValues) {
  ...
  const char* args[] = {"--test", "foo", "--test", "meh"};
  const int num_args = sizeof(args) / sizeof(args[0]);
  const bool result = parser.Parse(num_args, args);
  EXPECT_TRUE(result);
...
TEST(ArgParserTest, GetValues) {
  ...
  const char* args[] = {"--test", "foo", "--test", "meh", "--moo", "woo"};
  const int num_args = sizeof(args) / sizeof(args[0]);
  const bool result = parser.Parse(num_args, args);
  EXPECT_TRUE(result);
