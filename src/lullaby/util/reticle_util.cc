...
#include "lullaby/util/reticle_util.h"
...
bool GetReticleIntersectionPoint(Registry* registry, Entity entity,
  ...
  const Ray collision_ray = registry->Get<ReticleSystem>()->GetCollisionRay();
...
Sqt AdjustSqtForReticle(Registry* registry, const Sqt& sqt) {
  ...
  auto* reticle_system = registry->Get<ReticleSystem>();
  if (!reticle_system) {
    LOG(DFATAL) << "AdjustSqtForReticle depends on ReticleSystem";
    return sqt;
  }
  ...
  const Sqt* reticle_sqt =
      transform_system->GetSqt(reticle_system->GetReticle());
  if (!reticle_sqt) {
    return sqt;
  }
