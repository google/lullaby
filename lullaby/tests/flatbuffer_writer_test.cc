...
TEST(FlatbufferWriter, FileIdentifier) {
  ComplexT obj;
  ...
  InwardBuffer buffer(32);
  const void* flatbuffer = WriteFlatbuffer(&obj, &buffer, "ABCD");
  auto char_data = static_cast<const char*>(flatbuffer);

  // Test file identifier.
  EXPECT_THAT(char_data[4], Eq('A'));
  EXPECT_THAT(char_data[5], Eq('B'));
  EXPECT_THAT(char_data[6], Eq('C'));
  EXPECT_THAT(char_data[7], Eq('D'));

  // Verify flatbuffer is ok.
  const Complex* c = flatbuffers::GetRoot<Complex>(flatbuffer);
  ...
  // Test file identifier generated from definition file.
  testing::ComplexDefT obj1;
  flatbuffer = WriteFlatbuffer(&obj1, &buffer);
  char_data = static_cast<const char*>(flatbuffer);
  EXPECT_THAT(char_data[4], Eq('L'));
  EXPECT_THAT(char_data[5], Eq('U'));
  EXPECT_THAT(char_data[6], Eq('L'));
  EXPECT_THAT(char_data[7], Eq('L'));
}
