...
TEST(ScriptEnvTest, Lambda) {
  ScriptEnv env;
  ScriptValue res =
      env.Exec("(do (= foo (lambda (x y) (+ x (+ y y)))) (foo 1 2))");
  EXPECT_THAT(res.Is<int>(), Eq(true));
  EXPECT_THAT(*res.Get<int>(), Eq(5));
}
...
TEST(ScriptEnvTest, PassLambda) {
  ...
  ScriptValue res = env.Exec(
      "(do"
      " (def foo (fn x y) (fn x y))"
      " (foo (lambda (x y) (+ x y)) 1 2))");
  EXPECT_THAT(res.Is<int>(), Eq(true));
...
TEST(ScriptEnvTest, CallLambdaWithArray) {
  ScriptEnv env;
  ScriptValue res = env.Exec("(lambda (x y) (+ x y))");
  ASSERT_THAT(res.Is<Lambda>(), Eq(true));
  Lambda* lambda = res.Get<Lambda>();

  std::vector<ScriptValue> args;
  args.push_back(env.Create(1));
  args.push_back(env.Create(2));
  res = env.CallWithArray(*lambda, args);
  EXPECT_THAT(res.Is<int>(), Eq(true));
  EXPECT_THAT(*res.Get<int>(), Eq(3));
}

TEST(ScriptEnvTest, CallLambdaWithMap) {
  ScriptEnv env;
  ScriptValue res = env.Exec("(lambda (x y) (+ x y))");
  ASSERT_THAT(res.Is<Lambda>(), Eq(true));
  Lambda* lambda = res.Get<Lambda>();
  ...
  VariantMap kwargs;
  kwargs[ConstHash("x")] = 2;
  kwargs[ConstHash("y")] = 4;
  res = env.CallWithMap(*lambda, kwargs);
  EXPECT_THAT(res.Is<int>(), Eq(true));
...
TEST(ScriptEnvTest, CallRegisteredFunction) {
  ScriptEnv env;
  env.Register("foo", [](IContext* context) {
    if (!CallNativeFunction(context, "foo",
...
TEST(ScriptEnvTest, DoReturnNoArg) {
  ScriptEnv env;
  ScriptValue res;

  res = env.Exec("(do (return) 2)");
