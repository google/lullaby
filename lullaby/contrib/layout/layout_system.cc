...
LayoutSystem::LayoutSystem(Registry* registry)
    : System(registry),
      layouts_(8),
      ignored_layout_elements_(16),
      layout_elements_(16) {
  RegisterDef<LayoutDefT>(this);
  RegisterDef<LayoutElementDefT>(this);
  RegisterDef<RadialLayoutDefT>(this);
...
void LayoutSystem::Create(Entity e, HashValue type, const Def* def) {
  if (type == kLayoutElementDef) {
    ...
    if (data->ignored()) {
      ignored_layout_elements_.emplace(e);
    }
...
void LayoutSystem::Destroy(Entity e) {
  layouts_.Destroy(e);
  ignored_layout_elements_.erase(e);
...
void LayoutSystem::SetElementIgnored(Entity element, bool ignored) {
  if (ignored) {
    ignored_layout_elements_.emplace(element);
  } else {
    ignored_layout_elements_.erase(element);
  }
  SetParentDirty(element, kOriginal);
}
...
void LayoutSystem::LayoutImpl(const DirtyLayout& dirty_layout) {
  ...
  if (layout->layout) {
    ...
    for (const Entity& child : *children) {
      ...
      if (ignored_layout_elements_.count(child) > 0) {
        continue;
      }
