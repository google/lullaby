...
TEST_F(DebugCameraTest, StartStopAndCameraMovement) {
  ...
  input_manager_->ConnectDevice(device, params);
  ...
  // equal to delta time.
  EXPECT_THAT(DistanceBetween(start_position, fling_while_debugging_position),
  ...
  EXPECT_THAT(DistanceBetween(start_position, after_debugging_position),
              FloatEq(0));
}
