...
static std::unique_ptr<OpusReader> CreateOpusReader(std::string_view uri) {
  return std::make_unique<OpusReader>(CreateDataReader(uri));
}

TEST(OpusReaderTests, BasicInfo) {
  auto reader = CreateOpusReader("speech.opus");

  EXPECT_TRUE(reader->IsValid());
  EXPECT_THAT(reader->GetNumChannels(), Eq(1));
  EXPECT_THAT(reader->GetSampleRateHz(), Eq(8000));
  EXPECT_THAT(reader->GetTotalFrameCount(), Eq(142368L));
  EXPECT_THAT(reader->GetReadFramePosition(), Eq(0));
}

TEST(OpusReaderTests, Decode) {
  auto reader = CreateOpusReader("speech.opus");
...
TEST(OpusReaderTests, DecodeSmallFile) {
  auto reader = CreateOpusReader("small.opus");
...
TEST(OpusReaderTests, CheckHeader) {
  auto reader1 = CreateDataReader("speech.opus");
  EXPECT_TRUE(OpusReader::CheckHeader(reader1));

  auto reader2 = CreateDataReader("speech.wav");
  EXPECT_FALSE(OpusReader::CheckHeader(reader2));
}
