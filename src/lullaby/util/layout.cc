...
using UpdateFunction =
    std::function<void (Entity entity, mathfu::vec2 pos, mathfu::vec2 size)>;
...
UpdateFunction ApplyLayoutUpdateFunction(TransformSystem* transform_system,
                                         ...
                                         const mathfu::vec2* root_pos,
                                         bool* min_max_set,
                                         mathfu::vec2* min,
                                         mathfu::vec2* max) {
  return [=](Entity entity, mathfu::vec2 pos, mathfu::vec2 size) {
...
Aabb ApplyLayout(Registry* registry, const LayoutParams& params,
  ...
  const auto update_root_x_fn = [&](mathfu::vec2i pos, mathfu::vec2i size) {
    const mathfu::vec2 virtual_pos =
        layout.PhysicalToVirtual(pos) / kScaleFactor;
    root_pos.x = virtual_pos.x;
  };
  const auto update_root_y_fn = [&](mathfu::vec2i pos, mathfu::vec2i size) {
    const mathfu::vec2 virtual_pos =
        layout.PhysicalToVirtual(pos) / kScaleFactor;
      ...
      for (size_t i = 0; i < range.size; ++i) {
        ...
        layout.Element(size, entity,
                       [&](mathfu::vec2i pos, mathfu::vec2i size) {
                         ...
                         update_fn(entity, virtual_pos, virtual_size);
                       });
      }
