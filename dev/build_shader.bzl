...
def build_shader(
  ...
  _build_shader_internal(name, srcs, out, version, defines,
                                 include_deps, strip_prefix)
  ...
  if (build_multiview):
    ...
    _build_shader_internal(name, srcs, multiview_out, multiview_version,
                                   multiview_defines,
                                   include_deps, strip_prefix)
    native.filegroup(name="%s_only_multiview" % name,
                     srcs=[multiview_out],
                     visibility=visibility,
                    )
    native.filegroup(name="%s_with_multiview" % name,
...
def _build_shader_internal(
  ...
  native.genrule(name="%s_%s_genrule" % (name, out),
...
def build_lullaby_shader(
  ...
  shader_schema_path = "third_party/lullaby/schemas/lull/shader_def.fbs"
  shader_compiler_path = "//lullaby/tools/shader_pipeline"
  fbs_schema_includes = ["//:fbs_schema_includes"];
  ...
  native.genrule(name = "%s_%s_genrule" % (name, out),
                 srcs = srcs_vertex + srcs_fragment + include_deps +
                     ...
                     ["//third_party/lullaby/data:jsonnet_files"],
                 outs = [out],
