...
#include "lullaby/contrib/cursor/cursor_trail_system.h"
#include <cmath>
...
CursorTrailSystem::CursorTrailSystem(Registry* registry)
  ...
  RegisterDef<CursorTrailDefT>(this);
  ...
  cursor_trails_.ForEach([this, transform_system,
    ...
    if (SecondsFromDuration(delta_time) > kMaxDeltaTime) {
      ...
      cursor_trail.trail_length =
          std::min(1 + static_cast<int>(std::floor(curr_speed_length)),
...
void CursorTrailSystem::UpdateTrailMesh(CursorTrail* cursor_trail, Sqt sqt) {
  ...
  auto trail_fn = [cursor_trail, cursor_color, camera_position, no_hit_distance,
                   sqt](MeshData* mesh) {
    ...
    for (int i = 0; i < cursor_trail->trail_length; i++) {
      ...
      v.color = Color4ub::FromVec4(color);
      v.u0 = 0.0f;
      v.v0 = 1.0f;
