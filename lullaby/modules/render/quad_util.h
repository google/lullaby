...
#ifndef LULLABY_UTIL_QUAD_UTIL_H_
#define LULLABY_UTIL_QUAD_UTIL_H_
...
// read+write access.
template <typename Vertex>
MeshData CreateQuadMesh(float size_x, float size_y, int num_verts_x = 2,
                        int num_verts_y = 2, float corner_radius = 0.f,
                        int corner_verts = 0,
                        CornerMask corner_mask = CornerMask::kAll) {
...
// Creates the vertices and indices for a quad and adds it to mesh.
inline void CreateQuad(uint32_t vert_index, const mathfu::vec3& upper_right,
                       const mathfu::vec3& upper_left,
                       const mathfu::vec3& lower_right,
                       const mathfu::vec3& lower_left,
                       const mathfu::vec2& uv_min, const mathfu::vec2& uv_max,
                       lull::MeshData* mesh) {
  mesh->AddVertex<lull::VertexPT>(upper_right,
                                  mathfu::vec2(uv_max.x, uv_min.y));
  mesh->AddVertex<lull::VertexPT>(upper_left, uv_min);
  mesh->AddVertex<lull::VertexPT>(lower_right, uv_max);
  mesh->AddVertex<lull::VertexPT>(lower_left, mathfu::vec2(uv_min.x, uv_max.y));

  // Create two triangles with a counter-clockwise winding order (lullaby's
  // default for front facing). The first triangle is upper right, upper left,
  // lower right.
  mesh->AddIndex(vert_index + 0);
  mesh->AddIndex(vert_index + 1);
  mesh->AddIndex(vert_index + 2);
  // The second triangle is lower right, upper left, lower left completing the
  // quad.
  mesh->AddIndex(vert_index + 2);
  mesh->AddIndex(vert_index + 1);
  mesh->AddIndex(vert_index + 3);
}

// Creates the vertices and indices for a quad and adds it to mesh.
inline void CreateQuad(uint32_t vert_index, float halfSize,
                       const mathfu::vec3& up, const mathfu::vec3& right,
                       const mathfu::vec3& position, const mathfu::vec2& uv_min,
                       const mathfu::vec2& uv_max, lull::MeshData* mesh) {
  const mathfu::vec3 upper_right = position + (up + right) * halfSize;
  const mathfu::vec3 upper_left = position + (up - right) * halfSize;
  const mathfu::vec3 lower_right = position + (-up + right) * halfSize;
  const mathfu::vec3 lower_left = position + (-up - right) * halfSize;
  CreateQuad(vert_index, upper_right, upper_left, lower_right, lower_left,
             uv_min, uv_max, mesh);
}

}  // namespace lull

#endif  // LULLABY_UTIL_QUAD_UTIL_H_
